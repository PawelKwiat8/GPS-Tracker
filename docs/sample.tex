\documentclass[12pt, a4paper]{EE-dyplom}
...
\title{Projekt i realizacja autonomicznego rejestratora parametrów ruchu GPS z interfejsem bezprzewodowym opartym o układ ESP32}
\album{320126}
\author{Paweł Kwiatkowski}
\promotor{[...]} 
\date{2026}

% Ręczna konfiguracja języka polskiego
...
\begin{abstract}
Celem niniejszej pracy inżynierskiej jest zaprojektowanie i realizacja autonomicznego 
rejestratora parametrów ruchu GPS, wyposażonego w zintegrowany interfejs bezprzewodowy WiFi. 
Urządzenie, oparte na dwurdzeniowym mikrokontrolerze ESP32, łączy w sobie funkcjonalność klasycznego
 "data-loggera" zapisującego dane na karcie pamięci SD z nowoczesną wizualizacją parametrów w czasie r
 zeczywistym poprzez dedykowaną stronę internetową (Web Interface).

W ramach projektu opracowano kompletną warstwę sprzętową, integrującą moduł nawigacji satelitarnej 
NEO-6M, czujnik inercyjny IMU (MPU6050) oraz wyświetlacz OLED. Praca opisuje również 
proces projektowania ergonomicznej obudowy  oraz jej wykonanie w technologii druku 3D. 
Przeprowadzone testy terenowe potwierdziły poprawność działania prototypu, stabilność rejestracji śladu 
oraz satysfakcjonujący czas pracy na zasilaniu bateryjnym.
\end{abstract}
\newpage
...
\newpage
\section{Wstęp}
\subsection{Cel pracy i założenia projektowe}

Celem tej pracy jest zaprojektowanie i 
zbudowanie urządzenia typu “GPS Tracker". Urządzenie ma rejestrować trasę,
monitorować parametry ruchu i pokazywać dane w czasie rzeczywistym.

Głównym założeniem projektu było stworzenie systemu autonomicznego, 
zasilanego bateryjnie, który łączy w sobie funkcjonalność klasycznego rejestratora danych 
 z nowoczesnym interfejsem użytkownika dostępnym przez przeglądarkę 
internetową (Web Interface).
...
\subsubsection{Wersja 1 (Prototyp)}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{../images/20260112_211048.png}
    \caption{Fizyczny model prototypu V1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../images/GPS_tracker_presentation.png}
    \caption{Widok rozstrzelony konstrukcji V1}
\end{figure}

\subsubsection{Wersja 2 (Finalna)}

W drugiej wersji wprowadzono szereg udoskonaleń wynikających z testów prototypu:
\begin{enumerate}
    \item \textbf{Uproszczona konstrukcja}: Usunięto zbędną klapkę oraz otwory na kartę SD, co zwiększyło sztywność i szczelność obudowy.
    \item \textbf{Zintegrowana pokrywa}: Dwie oddzielne górne pokrywy zostały połączone w jeden solidny element.
    \item \textbf{Wzmocnione ścianki}: Zwiększono grubość ścianek, co poprawiło wytrzymałość mechaniczną wydruku.
    \item \textbf{Lepsze dopasowanie}: Poprawiono spasowanie wszystkich elementów, eliminując luzy.
    \item \textbf{Dostęp do portów}: Zoptymalizowano otwory na porty USB (dla ładowania i programowania), ułatwiając podłączanie przewodów.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../images/GPS-TRACKER_V2.png}
    \caption{Widok złożonej obudowy V2}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../images/GPS-TRACKER_V2.2.png}
    \caption{Widok obudowy V2 z innej perspektywy}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../images/GPS-TRACKER_V2_PRESENTATION.png}
    \caption{Widok rozstrzelony konstrukcji V2}
\end{figure}
...
Testy wykazały, że o ile w warunkch, gdzie emodoł jest donbrze widoczny urządzenie radzi sobie bardzo dobrze, 
to podczas aktywności bardziej dynamicznych (np. bieg) 
lub przy okresowym przesłanianiu anteny, jakość sygnału zauważalnie spada.
...
\begin{enumerate}
    \item \textbf{Stabilność działania:} Dzięki wykorzystaniu dwóch rdzeni procesora ESP32, urządzenie działa płynnie. Nagrywanie trasy na kartę odbywa się niezależnie od obsługi WiFi, więc korzystanie z podglądu na telefonie nie zakłóca zapisu danych.
    \item \textbf{Prosty i szybki interfejs:} Strona WWW została napisana tak, aby nie obciążać procesora. Telefon pobiera tylko surowe liczby, a nie całą grafikę, dzięki czemu aplikacja działa szybko i nie zacina się.
    \item \textbf{Jakość modułu GPS:} Tani moduł NEO-6M jest wystarczający do turystyki pieszej i zastosowań amatorskich. W testach statycznych wykazuje wysoką precyzję, jednak w dynamicznym ruchu (bieg) i trudnym terenie (las) jego dokładność spada, co jest typowe dla odbiorników bez wspomagania inercyjnego.
    \item \textbf{Efektywność energetyczna:} Osiągnięty czas pracy na poziomie ok. 9 godzin przy zasilaniu z jednego ogniwa 18650 (z przetwornicą) czyni urządzenie praktycznym w jednodniowych zastosowaniach.
    \item \textbf{Obudowa 3D:} Zaprojektowana obudowa jest kompaktowa i funkcjonalna. Chroni elektronikę przed uszkodzeniami mechanicznymi, zapewniając jednocześnie łatwy dostęp do portu ładowania i karty pamięci.
\end{enumerate}

\subsection{Podsumowanie działania}
...
Dodatkową wartością projektu jest jego aspekt edukacyjny. 
Budowa urządzenia wymagała połączenia wiedzy z zakresu elektroniki,
 druku 3D oraz programowania systemów wbudowanych i aplikacji webowych. 
 Powstały prototyp jest nie tylko działającym urządzeniem, ale także solidną platformą 
 do dalszego rozwoju, np. poprzez dodanie modułu GSM w przyszłości.
