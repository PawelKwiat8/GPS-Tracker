...
// --- STAŁE ---
// Ustawienia Hotspotu (Telefonu) dla dostępu do Internetu
...
void setupWiFi() {
    // Proba polczenia z Hotspotem (aby miec internet na telefonie)
    ...
    // Konfiguracja AP (Backup lub local)
    // Uzywamy standardowej konfiguracji, telefon moze zapytac "Zachowac polaczenie?"
...
void setupServer() {
    ...
    server.on("/api/start", HTTP_GET, [](AsyncWebServerRequest *request){
        ...
        else if(currentState == PAUSED) currentState = RECORDING; // Resume
        request->send(200);
    });
    ...
    server.on("/api/pause", HTTP_GET, [](AsyncWebServerRequest *request){
        if(currentState == RECORDING) {
            currentState = PAUSED;
            pauseStart = millis(); // Track pause time
        }
        request->send(200);
    });
...
void displayLoop() {
    ...
    if(currentState == IDLE) {
        // Show IP in Idle so user knows where to connect
